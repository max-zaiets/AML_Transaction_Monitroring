...
**Domain:** Anti-Money Laundering · Compliance Analytics
...
| View | Description |
|---|---|
| `vw_account_drain` | Accounts fully emptied via TRANSFER or CASH_OUT |
...
Flags accounts where the full balance is sent out in a single TRANSFER or CASH_OUT (`OLD_BALANCE_ORIG > 0` and `NEW_BALANCE_ORIG = 0`). Normal customers rarely empty their accounts completely in one move — this pattern is typical of mule accounts being cleared after use.
...
5. **Rule calibration matters.** The account drain rule generates a high volume of alerts with relatively low precision. In a production environment, additional filters (minimum amount thresholds, account age, transaction frequency) would be needed to reduce noise and make the alert list actionable.
